...
function cluster_up() {
    rm -rf /mnt/efs/bsc-qa/benchmark/bsc_cluster
    mkdir -p /mnt/efs/bsc-qa/benchmark/bsc_cluster
    cp -r ${workspace}/.local/bsc/* /mnt/efs/bsc-qa/benchmark/bsc_cluster/
    cp -r ${workspace}/stop_geth.sh /mnt/efs/bsc-qa/benchmark/bsc_cluster/
    cp -r ${workspace}/start_geth.sh /mnt/efs/bsc-qa/benchmark/bsc_cluster/
    cp -f ${workspace}/bin/geth /mnt/efs/bsc-qa/benchmark/bsc_cluster/
    ...
    for ((i = 0; i < ${#bsc_node_ips[@]}; i++)); do
        ...
        aws ssm send-command \
            ...
            --parameters commands="mkdir -p /server/bsc/ && yes | cp -f /mnt/efs/bsc-qa/benchmark/bsc_cluster/stop_geth.sh /server/bsc/stop_geth.sh && yes | cp -f /mnt/efs/bsc-qa/benchmark/bsc_cluster/start_geth.sh /server/bsc/start_geth.sh && sudo bash /server/bsc/stop_geth.sh"
    done
    sleep 10
    for ((i = 0; i < ${#bsc_node_ips[@]}; i++)); do
        dst_id=${ips2ids[${bsc_node_ips[i]}]}
...
function cluster_restart() {
    yes | cp -r ${workspace}/stop_geth.sh /mnt/efs/bsc-qa/benchmark/bsc_cluster/
    yes | cp -r ${workspace}/start_geth.sh /mnt/efs/bsc-qa/benchmark/bsc_cluster/
...
function fyenman_hardfork(){
    ...
    yes | cp -f ${workspace}/tmp/bsc-feynman/bsc/build/bin/geth /mnt/efs/bsc-qa/benchmark/bsc_cluster/
...
function migrate_validator() {
    ...
    rm -rf /mnt/efs/bsc-qa/benchmark/bsc_cluster/clusterNetwork/node${validator_index}/keystore
    rm -rf /mnt/efs/bsc-qa/benchmark/bsc_cluster/clusterNetwork/node${validator_index}/bls
    mkdir -p /mnt/efs/bsc-qa/benchmark/bsc_cluster/clusterNetwork/node${validator_index}/keystore
    mkdir -p /mnt/efs/bsc-qa/benchmark/bsc_cluster/clusterNetwork/node${validator_index}/bls
    yes | cp -rf  ${workspace}/.local/bsc/new_validator${validator_index}/keystore/* /mnt/efs/bsc-qa/benchmark/bsc_cluster/clusterNetwork/node${validator_index}/keystore/
    yes | cp -rf  ${workspace}/.local/bsc/new_validator${validator_index}/bls/* /mnt/efs/bsc-qa/benchmark/bsc_cluster/clusterNetwork/node${validator_index}/bls/
    ...
    # wait epoch sync
    echo "wait for epoch sync validator set $((BSC_BLCOK_INTERVAL * BSC_EPOCH + 10))"
    sleep $((BSC_BLCOK_INTERVAL * BSC_EPOCH + 10))
}
