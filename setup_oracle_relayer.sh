...
function exit_previous() {
    for ((i = 0; i < ${#bc_node_ips[@]}; i++)); do
        ...
        aws ssm send-command \
          ...
          --parameters commands="mkdir -p /server/relayer/ && cp /mnt/efs/bsc-qa/benchmark/relayer/stop_oracle_relayer.sh /server/relayer/ && bash /server/relayer/stop_oracle_relayer.sh"
    done
}
...
function cluster_up() {
    ...
    for ((i = 0; i < ${#bc_node_ips[@]}; i++)); do
        ...
        aws ssm send-command \
          --instance-ids "${dst_id}" \
          --document-name "AWS-RunShellScript" \
          --parameters commands="mkdir -p /server/relayer/ && cp /mnt/efs/bsc-qa/benchmark/relayer/start_oracle_relayer.sh /server/relayer/ && bash /server/relayer/start_oracle_relayer.sh ${i} reset"
    done
}
