...
function start_cluster() {
    rm -rf /mnt/efs/bsc-qa/benchmark/bc_cluster
    mkdir -p /mnt/efs/bsc-qa/benchmark/bc_cluster
    cp -r ${workspace}/.local/bc/* /mnt/efs/bsc-qa/benchmark/bc_cluster/
    cp -r ${workspace}/stop_node.sh /mnt/efs/bsc-qa/benchmark/bc_cluster/
    cp -r ${workspace}/start_node.sh /mnt/efs/bsc-qa/benchmark/bc_cluster/
    cp -f ${workspace}/bin/bnbchaind /mnt/efs/bsc-qa/benchmark/bc_cluster/
    ...
    for ((i = 0; i < ${#bc_node_ips[@]}; i++)); do
        ...
        aws ssm send-command \
            ...
            --parameters commands="mkdir -p /server/bc/ && yes | cp -f /mnt/efs/bsc-qa/benchmark/bc_cluster/stop_node.sh /server/bc/stop_node.sh && yes | cp -f /mnt/efs/bsc-qa/benchmark/bc_cluster/start_node.sh /server/bc/start_node.sh && sudo bash /server/bc/stop_node.sh"
    done
    sleep 10
    for ((i = 0; i < ${#bc_node_ips[@]}; i++)); do
        dst_id=${ips2ids[${bc_node_ips[i]}]}
...
function cluster_restart() {
    yes | cp -r ${workspace}/stop_node.sh /mnt/efs/bsc-qa/benchmark/bc_cluster/
    yes | cp -r ${workspace}/start_node.sh /mnt/efs/bsc-qa/benchmark/bc_cluster/
    yes | cp -f ${workspace}/bin/bnbchaind /mnt/efs/bsc-qa/benchmark/bc_cluster/
...
function first_sunset_hardfork() {
    ...
    for ((i = 0; i < ${#bc_node_ips[@]}; i++)); do
        ...
        yes | cp -f ${workspace}/.local/bc/node${i}/config/app.toml /mnt/efs/bsc-qa/benchmark/bc_cluster/node${i}/config/app.toml
    done
