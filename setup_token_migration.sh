...
function start_approval_service() {
    mkdir -p /mnt/efs/bsc-qa/benchmark/approval_service

    yes | cp -rf ${TOKEN_APPROVER_BIN} /mnt/efs/bsc-qa/benchmark/approval_service/approver
    yes | cp -rf ${workspace}/start_token_approver.sh /mnt/efs/bsc-qa/benchmark/approval_service/start_token_approver.sh
    yes | cp -rf ${workspace}/stop_token_approver.sh /mnt/efs/bsc-qa/benchmark/approval_service/stop_token_approver.sh

    aws ssm send-command \
      --instance-ids "${dst_id}" \
      --document-name "AWS-RunShellScript" \
      --parameters commands="mkdir -p /server/token_approver/ && yes | cp -rf /mnt/efs/bsc-qa/benchmark/approval_service/start_token_approver.sh /server/token_approver/start_token_approver.sh && bash /server/token_approver/start_token_approver.sh"
}
...
function stop_approval_service() {
    ...
    aws ssm send-command \
      --instance-ids "${dst_id}" \
      --document-name "AWS-RunShellScript" \
      --parameters commands="mkdir -p /server/token_approver/ && yes | cp -rf /mnt/efs/bsc-qa/benchmark/approval_service/stop_token_approver.sh /server/token_approver/stop_token_approver.sh && bash /server/token_approver/stop_token_approver.sh"
}
